...
본 문서는 `scripts/ins_v1_0_3.py`로 학습된 **InsureGuard AI v1.0.3**
...
| API 서빙 | `src/inference.py` → `POST /predict` (기본 v1.0.3) |

현재 pkl에 저장된 학습 지표:

| 지표 | 값 | 의미 |
|------|-----|------|
| **R²** | **0.985** | 프로파일 위험점수 회귀 적합도 |
| RMSE | 2.43 | 0~100 점수 스케일 오차 |
| MAE | 1.51 | 평균 절대 오차 |
| 법규위반 Acc. | 54.6% | 개별 사고 법규 라벨 맞히기 (별도 과제) |
| 사고경중 Acc. | 68.0% | 개별 사고 경중 라벨 맞히기 (별도 과제) |

가중치 상수(`scripts/ins_v1_0_3.py`):
...
[v1.0.3]  ← 본 문서
  X = 성별·연령·차종·지역(+교차)
  y = 동일 프로파일의 기대위험(심각도+빈도, 행마다 동일) →  R² ~ 0.985
```
...
- 프로파일 통계(`smooth_epdo`, \(n_g\) 등)는 **전체 데이터로 한 번 집계한 뒤** split 합니다.  
  ...
  → R² 0.985는 “프로파일 점수 재현력”이지, “미지의 개별 사고 경중 예측력”이 아닙니다.
...
1. **대외·보고서:** “R² 0.985 = 사고 예측 정확도 98.5%”로 쓰지 말 것.  
   → “프로파일 위험 스코어카드 재현 R² ≈ 0.985”로 표기.
2. **더 엄격한 검증이 필요하면:**  
...
| 파일 | 역할 |
|------|------|
| `scripts/ins_v1_0_3.py` | v1.0.3 학습·추론·그래프 |
