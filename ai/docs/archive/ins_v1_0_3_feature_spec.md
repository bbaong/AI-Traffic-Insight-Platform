...
# InsureGuard AI v1.0.3 — 피처 명세서 & R² 해석 가이드

본 문서는 `scripts/archive/ins_v1_0_3.py`로 학습된 **InsureGuard AI v1.0.3**
(`models/ins_model_v1.0.3.pkl`)의 입력·타깃 정의와,
...
> **데이터 갱신(2026-08):** `사고분석-군위군.csv`(2016.1~2023.6)를 원본합본에 병합한 뒤 **동일 스크립트로 재학습**.  
> 알고리즘은 v1.0.3 그대로이며, 군위 관측 기간이 2.5년 → 10년으로 맞춰졌다. 전후 비교는 [§8](#8-이전-버전이전-학습-대비) 참고.
...
가중치 상수(`scripts/archive/ins_v1_0_3.py`):
...
| | v1 (`new_model.py`) | v1.0.3 (`ins_v1_0_3.py`) |
|--|---------------------|---------------------------|
...
2. **더 엄격한 검증이 필요하면:**  
   ...
   → **A~C 실행 결과:** `docs/validation_v1_0_3.md`  
...
알고리즘 변경(v1 → v1.0.2 → v1.0.3)과, **같은 v1.0.3 스크립트**로 군위 시계열을 보강한 재학습을 구분한다.

### 8.1. 알고리즘 계열

| | v1 (`new_model.py`) | v1.0.2 | **v1.0.3 (현재)** |
|--|---------------------|--------|-------------------|
| 입력 | 연령, 성별, 차종, 지역, 주야, 노면 | 연령, 성별, 차종, 지역 | 동일 (4개 + 교차 6) |
| 타깃 | 개별 행 심각도 × 발생률 → 백분위 | 프로파일 **심각도만** 백분위 | 프로파일 **심각도 70% + 빈도 30%** |
| 같은 입력의 y | 사고마다 크게 흔들림 | 프로파일마다 고정 | 프로파일마다 고정 |
| R² 해석 | 개별 위험 근사 (~0.4) | 스코어카드 재현 (~0.95–0.99) | 스코어카드 재현 (~0.987) |
| 흔한 프로파일 | 심각도만 보면 낮을 수 있음 | 그대로 낮게 나옴 | 빈도 순위로 점수 보정 |

v1.0.2 → v1.0.3은 “더 똑똑한 모델”이 아니라 **타깃에 빈도를 넣은 스코어카드 재정의**다.

### 8.2. v1.0.3 재학습 — 군위 2016.1~2023.6 추가

편입 전 군위 사고(`사고분석-군위군.csv`, 경상북도 표기 808건)를 원본합본에 합친 뒤 `ins_v1_0_3.py`로 **pkl을 다시 학습**했다. 피처·가중치·블렌드 상수는 그대로다.

| 항목 | 이전 학습 (v1.0.3 최초) | **이번 재학습** |
|------|-------------------------|-----------------|
...
| 파일 | 역할 |
|------|------|
| `scripts/archive/ins_v1_0_3.py` | v1.0.3 학습·추론·그래프 |
| `models/ins_model_v1.0.3.pkl` | v1.0.3 패키지 (있을 경우) |
| `docs/ins_v1_0_4_feature_spec.md` | 현재 서빙 v1.0.4 |
