"""GovGuard AI v1.0.5 추론 — share×시전체 + last×2 캡 건수 (+EB 중대 보조)."""
...
MODEL_PATH = MODEL_DIR / "gov_model_v1.0.5.pkl"
SCRIPT_PATH = Path(__file__).resolve().parent.parent / "scripts" / "gov_v1_0_5.py"
...
def predict_gov_rates(
    ...
    *,
    ...
    """다음 기간 예상 사고건수(메인: share×캡) + 점유율 + EB 중대(보조).
...
def predict_gov_history(
    ...
    mod = _gov_mod()
    package = load_model()
    return mod.predict_quarter_history(
        package,
        지역=지역,
        n_history=n_history,
        as_of_연도분기=as_of,
