...
def _train_models_core(df: pd.DataFrame | None = None) -> dict:
...
def _predict_next_quarter_core(
    package: dict,
    지역: str | None = None,
...
# ---------------------------------------------------------------------------
# v1.0.4 서빙 정책 (캡 · 유형비율 · history)
# ---------------------------------------------------------------------------


def _rp_key(지역: str, 연도분기: str) -> str:
    return f"{지역}|{연도분기}"


def _normalize_type_shares(counts: dict[str, float]) -> dict[str, float]:
...
def build_type_share_tables(df: pd.DataFrame) -> dict:
    """학습용: 원본 사고 DF → pkl 유형 비율 테이블."""
    empty = {
        "type_order": TYPE_ORDER,
        "type_share_by_region_period": {},
        "type_share_by_region": {},
    }
    if "사고유형" not in df.columns:
        return empty
    work = df.copy()
    work["사고유형대분류"] = work["사고유형"].astype(str).str.split(" - ").str[0]
    work = work[work["사고유형대분류"].isin(TYPE_ORDER)]
    if work.empty:
        return empty

    by_rp: dict[str, dict[str, float]] = {}
    grp = (
        work.groupby(["지역", "연도분기", "사고유형대분류"], as_index=False)
        .size()
    ...
    for (region, period), sub in grp.groupby(["지역", "연도분기"]):
        ...
        by_rp[_rp_key(str(region), str(period))] = _normalize_type_shares(counts)
    ...
    for region, sub in grp_r.groupby("지역"):
        ...
        for _, row in sub.iterrows():
            counts[str(row["사고유형대분류"])] = float(row["건수"])
        by_region[str(region)] = _normalize_type_shares(counts)

    return {
        "type_order": TYPE_ORDER,
        "type_share_by_region_period": by_rp,
        "type_share_by_region": by_region,
    }


def type_shares_for(
    package: dict, 지역: str | None, 기준분기: str | None
) -> dict[str, float]:
    by_rp = package.get("type_share_by_region_period") or {}
    by_region = package.get("type_share_by_region") or {}
    if 지역 and 기준분기:
        hit = by_rp.get(_rp_key(str(지역), str(기준분기)))
        if hit:
            return dict(hit)
    if 지역:
        hit = by_region.get(str(지역))
        if hit:
            return dict(hit)
    return _normalize_type_shares({k: 1.0 for k in TYPE_ORDER})
...
def _apply_row_policy(
    row: dict, package: dict, *, count_reg_value: int | None = None
) -> dict:
    out = dict(row)
    ...
    shares = type_shares_for(package, out.get("지역"), out.get("기준분기"))
    out["예측사고유형비율"] = {k: round(v, 4) for k, v in shares.items()}
    out["예측사고유형_퍼센트"] = {k: round(v * 100, 2) for k, v in shares.items()}
    out["사고유형_출처"] = "기준분기_실적비율_전파(pkl)"
    return out


def _finalize_v104_package(package: dict, raw: pd.DataFrame) -> dict:
    ...
    package["serving"] = {
        ...
        "type_shares": "embedded_in_pkl_no_csv_at_serve",
    }
    package.update(build_type_share_tables(raw))
    return package


def train_models(df: pd.DataFrame | None = None) -> dict:
    raw = df if df is not None else load_raw()
    package = _train_models_core(raw)
    return _finalize_v104_package(package, raw)
...
def predict_next_quarter(
    ...
    """B1 건수 + last×2 캡 메인. 서빙 시 CSV 불필요."""
    raw = _predict_next_quarter_core(
        package, 지역=지역, as_of_연도분기=as_of_연도분기
    )
    if isinstance(raw, dict):
        return _apply_row_policy(raw, package)
    rows = [_apply_row_policy(r, package) for r in raw]
    rows.sort(key=lambda r: r["예측사고건수"], reverse=True)
    return rows
...
def predict_quarter_history(
    ...
    sev_order = list(package.get("severity_order") or SEVERITY_ORDER)
    panel = package["latest_panel"].copy()
        ...
        history.append(
            {
                ...
                "경중_건수": {col: int(cur[col]) for col in sev_order},
                "경중_퍼센트": {
    ...
    forecast = {
        ...
        "예측사고유형_퍼센트": forecast_row.get("예측사고유형_퍼센트"),
    }
...
def main() -> None:
    ...
    print(f"서빙: share×시전체, 캡=min(., last×{LAST_COUNT_CAP_MULT})")
    raw = load_raw()
    ...
    print(
        f"   type_share periods={len(package.get('type_share_by_region_period') or {})}"
    )
    ...
    print(
        f"{'지역':<8} {'캡건수':>8} {'share':>8} {'last':>6} {'캡?':>4} {'중대%':>7}"
    )
    print("-" * 52)
    for r in preds:
        print(
