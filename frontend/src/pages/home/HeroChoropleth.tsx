...
export function HeroChoropleth({ selectedCode }: { selectedCode: string }) {
  ...
  const ordered = [...districts].sort((a, b) => {
    if (a.code === selectedCode) return 1;
    if (b.code === selectedCode) return -1;
    return 0;
  });
  ...
  return (
    <DashboardCard
      ...
      className={`${mapStyles.card} ${styles.mapCard}`}
    >
      <div className={`${mapStyles.mapWrap} ${styles.mapWrap}`}>
        <svg
        ...
        >
          {ordered.map((district) => {
            const risk: RiskLevel =
              DISTRICT_RISK_MOCK[district.code] ?? 'MODERATE';
        ...
        <ul className={mapStyles.legend} aria-hidden="true">
          ...
          <li className={mapStyles.legendItem}>
            <span
              className={mapStyles.swatchCircle}
              style={{
                borderColor: HOTSPOT_STROKE,
                background: HOTSPOT_FILL,
              }}
            />
            <span>사고 다발 지역</span>
          </li>
