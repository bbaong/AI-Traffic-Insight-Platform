import type { CustomerRiskGrade, ReportItem } from '../types/customers';

const SIX: Array<Pick<ReportItem, 'coverageKey' | 'coverageName'>> = [
...
/** 상담 id별 고정 목업 (2~3건 시연용). 실제 id는 등급 폴백 사용 */
export const MOCK_REPORT: Record<string, ReportItem[]> = {
  demo_low: LOW_REPORT,
  demo_moderate: MODERATE_REPORT,
  demo_high: HIGH_REPORT,
};

export function mockReportByGrade(
  grade: CustomerRiskGrade | null,
): ReportItem[] {
  if (grade === 'Low') return LOW_REPORT;
  if (grade === 'High') return HIGH_REPORT;
  if (grade === 'Critical') return CRITICAL_REPORT;
  if (grade === 'Moderate') return MODERATE_REPORT;
  return [];
}
